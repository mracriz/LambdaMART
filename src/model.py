...
Módulo principal do modelo LambdaMART com suporte para múltiplos frameworks.Módulo principal do modelo LambdaMART com suporte para múltiplos frameworks.

Factory pattern para XGBoost (com IPS) e LightGBM (baseline).Factory pattern para XGBoost (com IPS) e LightGBM (baseline).
...
from .model_factory import ModelFactoryfrom .model_factory import ModelFactory





class LambdaMARTModel:class LambdaMARTModel:

    """    """

    Wrapper principal para modelos LambdaMART.    Wrapper principal para modelos LambdaMART.

    Automaticamente escolhe XGBoost ou LightGBM baseado na configuração.    Automaticamente escolhe XGBoost ou LightGBM baseado na configuração.
    ...
    def __init__(self, params: Dict[str, Any]):    def __init__(self, params: Dict[str, Any]):
        ...
        Initialize LambdaMART model with automatic framework selection.        Initialize LambdaMART model with automatic framework selection.

                

        Args:        Args:

            params: Configuration dictionary with framework specification            params: Configuration dictionary with framework specification

        """        """

        self.config = params        self.config = params

        self.framework = params.get('framework', 'xgboost').lower()        self.framework = params.get('framework', 'xgboost').lower()

                

        # Create the appropriate model using factory        # Create the appropriate model using factory

        self.model = ModelFactory.create_model(params)        self.model = ModelFactory.create_model(params)

                

        print(f"🤖 Inicializando LambdaMART com framework: {self.framework.upper()}")        print(f"🤖 Inicializando LambdaMART com framework: {self.framework.upper()}")

                

        # Show IPS information for XGBoost        # Show IPS information for XGBoost

        if self.framework == 'xgboost' and hasattr(self.model, 'get_ips_info'):        if self.framework == 'xgboost' and hasattr(self.model, 'get_ips_info'):

            ips_info = self.model.get_ips_info()            ips_info = self.model.get_ips_info()

            if ips_info.get('ips_enabled', False):            if ips_info.get('ips_enabled', False):

                print("✅ IPS (Inverse Propensity Scoring) habilitado")                print("✅ IPS (Inverse Propensity Scoring) habilitado")

            else:            else:

                print("ℹ️ IPS (Inverse Propensity Scoring) desabilitado")                print("ℹ️ IPS (Inverse Propensity Scoring) desabilitado")

        elif self.framework == 'lightgbm':        elif self.framework == 'lightgbm':

            print("ℹ️ LightGBM baseline (sem IPS)")            print("ℹ️ LightGBM baseline (sem IPS)")

        

    def train(self, *args, **kwargs):    def train(self, *args, **kwargs):

        """Delegate training to the underlying model."""        """Delegate training to the underlying model."""

        return self.model.train(*args, **kwargs)        return self.model.train(*args, **kwargs)

        

    def predict(self, *args, **kwargs):    def predict(self, *args, **kwargs):

        """Delegate prediction to the underlying model."""        """Delegate prediction to the underlying model."""

        return self.model.predict(*args, **kwargs)        return self.model.predict(*args, **kwargs)

        

    def get_feature_importance(self, *args, **kwargs):    def get_feature_importance(self, *args, **kwargs):

        """Delegate feature importance to the underlying model."""        """Delegate feature importance to the underlying model."""

        return self.model.get_feature_importance(*args, **kwargs)        return self.model.get_feature_importance(*args, **kwargs)

        

    def save_model(self, *args, **kwargs):    def save_model(self, *args, **kwargs):

        """Delegate model saving to the underlying model."""        """Delegate model saving to the underlying model."""

        return self.model.save_model(*args, **kwargs)        return self.model.save_model(*args, **kwargs)

        

    def load_model(self, *args, **kwargs):    def load_model(self, *args, **kwargs):

        """Delegate model loading to the underlying model."""        """Delegate model loading to the underlying model."""

        return self.model.load_model(*args, **kwargs)        return self.model.load_model(*args, **kwargs)

        

    def get_model_summary(self):    def get_model_summary(self):

        """Get comprehensive model summary."""        """Get comprehensive model summary."""

        summary = self.model.get_model_summary()        summary = self.model.get_model_summary()

        summary['wrapper_framework'] = self.framework        summary['wrapper_framework'] = self.framework

        summary['config'] = self.config        summary['config'] = self.config

        return summary        return summary

        

    def print_ips_info(self):    def print_ips_info(self):

        """Print IPS information if available."""        """Print IPS information if available."""

        if hasattr(self.model, 'print_ips_info'):        if hasattr(self.model, 'print_ips_info'):

            self.model.print_ips_info()            self.model.print_ips_info()

        else:        else:

            print("ℹ️ IPS não disponível para este framework")            print("ℹ️ IPS não disponível para este framework")
...
def create_model(params: Dict[str, Any]):def create_model(params: Dict[str, Any]):

    """    """

    Create LambdaMART model (função de conveniência).    Create LambdaMART model (função de conveniência).

        

    Args:    Args:

        params: Configuration dictionary        params: Configuration dictionary

                

    Returns:    Returns:

        LambdaMARTModel instance        LambdaMARTModel instance

    """    """

    return LambdaMARTModel(params)    return LambdaMARTModel(params)
            'subsample': 0.8,
